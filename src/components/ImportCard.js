...
import { parseCsv, guessMapping, validateItems, validateStaff, importTargets, missingRequired } from "@/lib/import-parse";
import { apiImport } from "@/lib/data";

// Owner-only "Import / migrate" card (Phase 1: items, Phase 2: staff). Parses a
// CSV in the browser, lets the owner map columns onto DuoCount's fields, shows a
// live per-row dry-run preview (the same validators the server re-runs at
...
export default function ImportCard({ locations = [], items = [], staff = [], onToast }) {
  ...
  const report = useMemo(() => {
    ...
    return type === "items"
      ? validateItems(parsed.rows, mapping, { locations, existingItems: items, defaultLocationId })
      : validateStaff(parsed.rows, mapping, { locations, existingStaff: staff, defaultLocationId });
  }, [parsed, mapping, type, locations, items, staff, defaultLocationId]);
  ...
  const detail = (f) => type === "items"
    ? (f.locationName ? `${f.locationName}` : "")
    : [f.role, f.locationName || (f.role === "manager" ? "all locations" : ""), f.hasPin ? "sets PIN" : "no PIN", f.email].filter(Boolean).join(" · ");
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <label className="label">{type === "items" ? "Items" : "Staff"} CSV file</label>
                    ...
                    <table className="w-full text-[13px]">
                      <thead className="bg-panel text-muted text-[11px] uppercase tracking-wide sticky top-0">
                        <tr>
                          ...
                          <th className="text-left font-semibold px-2.5 py-2">{type === "items" ? "Item" : "Person"}</th>
