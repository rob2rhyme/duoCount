...
| **Scratch shift log** (the owner Scratch report gains the raw per-shift ledger the aggregates are built from: one row per pack per business day with the **opening ticket #** and the **closing ticket #** side by side, each carrying its server-pinned scan time and the staff member who signed it, plus tickets sold (`closing − opening`), sales, and a sold-out marker. Honest about incomplete shifts — opened-but-not-closed and a closing below the opening render the real numbers instead of a fabricated zero. Pure, unit-tested `buildShiftLog`; rendered on screen, in the branded print sheet, and as its own CSV section) | `src/lib/scratch-shift-log.js`, `scratch-analytics.js`, `ScratchReport.js`, `tests/scratch-shift-log.test.mjs` | ✅ |
