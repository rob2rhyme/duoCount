...
| **Scratch scan-to-log guardrails** (shift-aware duplicate-scan block on the "Scan to log" surface — the same game+pack+ticket is refused within a shift, Opening/Closing kept separate, survives a reload + a second device; a per-row **Sold out** action writes a signed `soldOut` final count; the replacement book of the same game # auto-reads as a fresh "New" book from #0 until the next day — all derived, no schema change) | `src/lib/scratch-scan-guard.js`, `src/components/ScratchForm.js`, `tests/scratch-scan-guard.test.mjs` | ✅ |
| **Owner-first game auto-fill everywhere** (one `resolveGameEntry`/`resolveGameByBarcode` resolver — the store's stored games first, then the bundled PA catalog — shared by the manual prefill, Shelf walk, **Scan to log**, and the Admin games card; Scan to log now logs any recognized game and refuses only a code in neither list, and scanning a ticket in Admin auto-fills the game name/price/pack-size) | `src/lib/scratch-catalog.js`, `src/components/ScratchForm.js`, `ScratchGamesCard.js`, `tests/scratch-catalog.test.mjs` | ✅ |
| **Developer soft-delete a store (with undo)** (dev-console `Delete store` — a reversible `status:"deleted"` + `deletedAt`/`deletedBy` written by the Admin-SDK `/api/dev` route, revoking staff sessions like suspend; the login route refuses a deleted store; an 8-second undo toast on `/dev` plus a persistent per-row **Restore** — no hard subtree purge, nothing erased; rules keep the flag server-only) | `src/app/api/dev/route.js`, `src/app/dev/page.js`, `src/app/api/auth/login/route.js`, `tests/rules.test.mjs` | ✅ |
| **Dependency security patch** (cleared all Dependabot advisories: `next` → 15.5.21 (8 CVEs), `sharp` → ≥0.35.0, `fast-uri`/`fast-xml-parser` patched, `@hono/node-server` ≥2.0.5 pinned via `overrides`; `npm audit` clean) | `package.json`, `package-lock.json` | ✅ |
| **Scratch-off report pages** (a per-staff/per-shift **History** view inside the Scratch tab + an owner-only **Scratch report** tab — KPI tiles, sales-over-time / top-games / by-staff charts (recharts, store-branded), per-staff/shift & by-game tables, theft-gap surfacing, CSV + branded print. A clerk sees only their own numbers; managers/owners see everyone. One shared pure `buildScratchAnalytics` engine keeps both views in lockstep. Scratch-off is **removed from the general Log and Reports** surfaces — the accounting journal keeps its lottery revenue) | `src/lib/scratch-analytics.js`, `src/components/ScratchHistory.js`, `ScratchReport.js`, `ReportModal.js`, `LogList.js`, `tests/scratch-analytics.test.mjs` | ✅ |
...
| **POS stock sync — Phase 1 (CSV) + expiry/low-stock alerts** (pos-inventory-sync-spec.md Phase 1: the owner-only **"Stock levels"** import type refreshes existing items' synced `quantity` / `price` / `expiresAt` from any POS export — update-only, item-resolution by name/barcode + location, re-running refreshes `quantitySyncedAt`; pure, unit-tested `validateStock`. `vendor.stockAlerts` owner settings (clamped: "Expiring soon" ≤ 30 days, "Need order" < 5 units) in Admin; the pure, unit-tested `stock-alerts.js` feeds a manager-only Dashboard **Stock attention** card (expired-first, en/es) and a digest email section. One-key rules change (`stockAlerts` on the vendor doc) — **emulator-verified**. The signed shift count is untouched) | `src/lib/stock-alerts.js`, `import-parse.js`, `api/import/route.js`, `ImportCard.js`, `AdminPanel.js`, `Dashboard.js`, `digest.js`, `firestore.rules`, `tests/stock-alerts.test.mjs` | ✅ |
| **Customer rewards — Phase 1 (core)** (rewards-program-spec.md Phase 1: phone-number enrollment at the register — no card, no app, no hardware; `customers` + the **append-only signed `rewardEvents` ledger** where every write goes through the trusted `/api/rewards` route (client rules allow **no** writes; points computed server-side from the owner's settings; the balance moves in the same transaction as the ledger line; owner-only `adjust` with a required note — corrections are new signed lines). Owner **Reward settings** in Admin ($1 = 1 pt, 100 pts = $5 off defaults, clamped, off by default) with the **live effective-%-back readout** + >2% caution and the legal exclusions note (tobacco/vape/alcohol/lottery/gift-cards/fuel). The **Rewards** tab (en/es): lookup by phone (masked display), enroll, earn on the qualifying sale, progress bar, one-tap redeem; employees see the tab only when the program is on. Pure, unit-tested `rewards.js`; `privacy-and-data.md` gains the rewards-customer disclosure (no marketing, no cross-merchant pooling). Rules change (`rewards` key + two read-only collections) — **emulator-verified** (members read, NOBODY writes client-side — owner included). No SMS (TCPA), no tiers; fraud detectors + liability readout are Phase 2) | `src/lib/rewards.js`, `api/rewards/route.js`, `RewardsPanel.js`, `AdminPanel.js`, `AppShell.js`, `BottomNav.js`, `require-manager.js`, `firestore.rules`, `tests/rewards.test.mjs`, `privacy-and-data.md` | ✅ |
| **Customer rewards — Phases 2–3 (trust + customer-facing)** (rewards-program-spec.md: **Phase 2** — the pure, unit-tested `reward-audit.js` detectors: points-issued reconciled against the **countersigned cash-sales totals** (the denominator no competitor has), per-customer multi-earn days (masked phone), per-clerk redemption bursts ("Name: …" titles ride the AI redactor's person list); alerts are pattern-shaped and join the Patterns card / digest / AI-insight flow untouched, plus the Dashboard "Rewards at a glance" row and digest line with the **outstanding-liability** figure. **Phase 3** — the public rate-limited **`/rewards` balance page** (store code + phone → points, en/es, never a name; every request counts against the login-style per-IP + per-store throttles) and the **printable bilingual counter sign** from Admin. The whole rules suite is now **emulator-verified** (73 tests, incl. the new keys + nobody-writes ledger collections). Category tagging → POS phase) | `src/lib/reward-audit.js`, `api/rewards/balance/route.js`, `BalanceCheck.js`, `Dashboard.js`, `digest.js`, `AdminPanel.js`, `tests/reward-audit.test.mjs`, `tests/rules.test.mjs` | ✅ |
| **Scratch report consolidation + dev store tools** (the Dashboard's scratch tiles/audit prose moved into the owner **Scratch report** tab — one scratch home; `/dev` Stores gains status/plan filters, search, and a CSV export of the store list) | `src/components/ScratchReport.js`, `Dashboard.js`, `src/app/dev/page.js` | ✅ |
| **Dev console accountability** (the `/dev` operator plane now meets the product's own standard: an **append-only `adminAudit` log** of every store action — actor, action, store, detail, server ts — surfaced in a `/dev` Audit tab; plus **operator identity + RBAC** — a server-only `platformAdmins/{uid}` registry mapping roles (superadmin / support / finance / readonly) to per-action scopes, enforced on every `/api/dev` action and mirrored in the console UI; deactivation is authoritative (never falls through to the env allowlist), removals tombstone, a transactional last-superadmin guard, and the env dev login stays break-glass) | `src/lib/admin-audit.js`, `src/lib/platform-admins.js`, `require-manager.js`, `api/dev/route.js`, `app/dev/page.js`, `firestore.rules`, `tests/platform-admins.test.mjs`, `tests/admin-audit.test.mjs` | ✅ |
| **Scratch theft-detection suite** (the after-hours pack-theft work: **server-pinned count timestamps** — `ts == request.time` on every count, so a count's "when" can't be backdated; a **mass pack-jump detector** — N books jumping in one shared window roll up into ONE alert (the batch-activation fingerprint) instead of a per-pack flood; an **off-shift count detector** — a scratch count signed while nobody was clocked in (store-level presence, fail-open without the time clock); the owner report's **per-boundary Opening#→Closing# ticket-sequence table** — numbers, staff, and server time on each side of every gap; and the **timestamped "stopped being counted" list** — last ticket #, signer, exact server time) | `src/lib/scratch-audit.js`, `patterns.js`, `data.js`, `firestore.rules`, `ScratchReport.js`, `digest.js`, `tests/scratch-audit.test.mjs`, `tests/patterns.test.mjs`, `tests/rules.test.mjs` | ✅ |
| **Books-on-hand census + reconcile** (closes the never-counted blind spot: staff scan every book on the shelf into a **signed, append-only, server-timed `scratchCensus` snapshot** (a Census view in the Scratch tab); the owner report reconciles censuses against the count trail — **walked** books (present before, gone now, never sold out) and **on-hand-but-never-counted** books (the freshly activated book a skim would hide). A physical-presence snapshot, not a pack-lifecycle revival) | `src/lib/scratch-census.js`, `src/components/ScratchCensus.js`, `ScratchReport.js`, `data.js`, `firestore.rules`, `tests/scratch-census.test.mjs` | ✅ |
| **Manager-countersign lever** (the anomalous scratch reads a thief would sign alone — a **gap-open** above the prior close, a **sellout-short** — surface as one rolled-up "needs a second signature" alert, with a one-tap **Countersign** on each row of the owner report's ticket-sequence table (reusing `verifiedBy`/`verifyOnly`; self-verify stays barred). Off-hours/mass-jump only *escalate* severity — a lone off-hours count is never flagged, since a shift-boundary count is structurally off-shift. Plus **`verifiedAt` is now server-pinned** (`== request.time`) so the countersign's "when" is as tamper-proof as the count's) | `src/lib/scratch-countersign.js`, `scratch-audit.js`, `patterns.js`, `ScratchReport.js`, `data.js`, `firestore.rules`, `tests/scratch-countersign.test.mjs` | ✅ |
| **Live settings + print/save UX** (the vendor doc is now a **live snapshot** in the session, so an owner's settings save lands on every signed-in device instantly — no re-login or app restart; the Admin **Save settings** buttons show a busy "Saving…" state instead of a silent await; every printed report window gets a floating **✕ Close** button — script-opened tabs had no way back on a phone — hidden on paper via `@media print`) | `src/components/SessionProvider.js`, `AdminPanel.js`, `src/lib/utils.js` `printCloseHtml`, the five print surfaces | ✅ |

| **Scratch shift log + counting policy** (the owner Scratch report AND the staff History view gain the raw per-shift ledger the aggregates are built from: one row per pack per business day with the **opening ticket #** and the **closing ticket #** side by side, each carrying its server-pinned scan time and the staff member who signed it, plus tickets sold (`closing − opening`), sales, and a sold-out marker. A manager/owner sees every clerk; a clerk sees the shifts they signed — rows PAIR across staff first, so a shift they only half-signed still appears (the other side's name is masked for them, see the staff-scope row below). Honest about incomplete shifts: a missing side and a closing below the opening render the real numbers instead of a fabricated zero. New owner setting **Scratch-off counting** (opening + closing / opening only / closing only) drives the count form's shift options and decides whether a one-sided day reads as incomplete. Pure, unit-tested `buildShiftLog` + `scratch-settings`; rendered on both screens, in the branded print sheet, and as a CSV section in both exports) | `src/lib/scratch-shift-log.js`, `scratch-settings.js`, `scratch-analytics.js`, `ScratchReport.js`, `ScratchHistory.js`, `ScratchForm.js`, `AdminPanel.js`, `firestore.rules`, `tests/scratch-shift-log.test.mjs` | ✅ |
| **Shift-log date banding** (the repeated per-row date column collapses into one heading per business day, with that day's tickets + sales totalled on the band — on the staff History view, the owner report, and the branded print sheet alike. `groupShiftLogByDate` is a single pass over the already-date-sorted rows, so it stays correct on a paged slice) | `src/lib/scratch-shift-log.js`, `ScratchHistory.js`, `ScratchReport.js`, `tests/scratch-shift-log.test.mjs` | ✅ |
| **Employee view scoping** (an employee's Dashboard KPIs, needs-attention queue, and by-employee table, their **Log** tab + CSV export, their Backroom movement history, and their Scratch History/Shift log now cover **only the counts they signed**; where a coworker is unavoidable — the other half of a paired shift, a backroom pull — the time and number show but the name reads "another staff member". Managers and owners are unchanged. New owner setting **What employees see** (`staffScope`: own / store) restores the old shared-log behaviour per store. Deliberately a **presentation** rule, applied at the render and never at the data source, because the arithmetic depends on cross-staff reads: a pack's opening ticket # chains off whoever took the previous reading. Pure, unit-tested `staff-scope.js` with fail-closed defaults) | `src/lib/staff-scope.js`, `Dashboard.js`, `LogList.js`, `ScratchHistory.js`, `ScratchForm.js`, `BackroomStock.js`, `AdminPanel.js`, `vendor-settings.js`, `firestore.rules`, `tests/staff-scope.test.mjs` | ✅ |
| **Printable-sheet section chooser** (the owner Scratch report's printout opens with a sticky **Include** strip — untick Summary / By game / By staff and they drop off the page, so a store that prints the sheet as a shift-handoff record keeps just the **Shift log**. Lives in the preview because that's where you notice the noise, remembers the choice per device, hides a section by `display:none` so re-ticking needs no reprint, offers no checkbox for a section with no rows, and never prints itself. Pure, unit-tested `print-sections.js`, escaped and fail-open — corrupt storage prints everything rather than a blank page) | `src/lib/print-sections.js`, `ScratchReport.js`, `tests/print-sections.test.mjs` | ✅ |
| **Scratch reorder reminder** (a manager nudge to order a fresh book before it sells out: when an active book's latest count is within its last few tickets — `remaining = perPack − endno`, owner threshold `vendor.scratch.reorderTickets`, default 5, `0` = off — the Dashboard **Order scratch books** card lists each game/book still selling and how many tickets remain. Pure, unit-tested `buildReorderAlerts` (newest count per pack, active books only, `perPack` from the count or bundled catalog), same visual language as Stock attention. A **Have a spare** dismiss parks a reminder in a manager-writable `scratchReorderDismissals/{packId}` doc — un-parking is a delete and a replacement book (new pack id) re-arms on its own; no lifecycle/settlement state. Owner threshold input in Admin; en/es in lockstep; rules emulator-verified) | `src/lib/scratch-reorder.js`, `scratch-settings.js`, `Dashboard.js`, `AppShell.js`, `AdminPanel.js`, `data.js`, `i18n.js`, `firestore.rules`, `tests/scratch-reorder.test.mjs`, `tests/rules.test.mjs` | ✅ |
