...
| **Scratch shift log + counting policy** (the owner Scratch report AND the staff History view gain the raw per-shift ledger the aggregates are built from: one row per pack per business day with the **opening ticket #** and the **closing ticket #** side by side, each carrying its server-pinned scan time and the staff member who signed it, plus tickets sold (`closing − opening`), sales, and a sold-out marker. A manager/owner sees every clerk; a clerk sees the shifts they signed — rows PAIR across staff first, so their row still names the coworker who took the other reading. Honest about incomplete shifts: a missing side and a closing below the opening render the real numbers instead of a fabricated zero. New owner setting **Scratch-off counting** (opening + closing / opening only / closing only) drives the count form's shift options and decides whether a one-sided day reads as incomplete. Pure, unit-tested `buildShiftLog` + `scratch-settings`; rendered on both screens, in the branded print sheet, and as a CSV section in both exports) | `src/lib/scratch-shift-log.js`, `scratch-settings.js`, `scratch-analytics.js`, `ScratchReport.js`, `ScratchHistory.js`, `ScratchForm.js`, `AdminPanel.js`, `firestore.rules`, `tests/scratch-shift-log.test.mjs` | ✅ |
