...
**Scratch-off counts.** Because lottery tickets are numbered in order, end number minus start number = tickets sold, times the ticket price = dollars that must be in the drawer. The math audits itself. Scanning a ticket fills the pack and the ticket number it's at, and a pack the store has counted before carries its game, price, and start number forward from the last close. Across shifts, the Dashboard's **Pack audit** checks each pack's opening ticket # against its previous closing # and names both signers at any break — the whole lottery feature is that shift-boundary theft check; settlement math stays with the state lottery.
