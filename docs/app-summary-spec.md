...
**Scratch-off counts.** Because lottery tickets are numbered in order, end number minus start number = tickets sold, times the ticket price = dollars that must be in the drawer. The math audits itself. Scanning a ticket fills the pack and the ticket number it's at, and a pack the store has counted before carries its game, price, and start number forward from the last close; scanning the same pack twice in one shift is refused, a sold-out book is finaled with one tap, and the replacement book of the same game opens fresh at #0. The Scratch tab has three views — the **count**, a per-staff **History**, and an occasional **Census** (scan every book physically on the shelf into one signed snapshot). The owner's **Scratch report** tab holds the analysis: sales by day/game/staff, plus the theft view — every break in a pack's ticket sequence with who signed each side and when (a server-set time that can't be backdated), a one-tap **Countersign** for a second manager, packs that stopped being counted, and the census reconciled against the counts so a book that walked — even one never counted — gets named. The whole lottery feature is that shift-boundary theft check; settlement math stays with the state lottery.

**Inventory counts.** A third kind of count, in the same signed log: shelf counts of high-shrink items (cigarette cartons, vapes, whatever the owner puts on the tracked list). Expected on hand = start + received − sold − removed, so a negative over/short means missing stock. It inherits verification, the shared log, per-location visibility, CSV export, and its own dashboard breakdown, with no special cases. Alongside the signed counts, owners can **sync stock levels from their POS** (a CSV import of on-hand quantity, price, and expiry per item), which drives two owner-tunable alerts on the Dashboard and in the daily digest: **Expiring soon** (default within 30 days) and **Need order** (default fewer than 5 left).

**Customer rewards (optional, off by default).** A phone-number points program at the register — no card, no app, no hardware. Owner-set economics (defaults $1 = 1 point, 100 points = $5 off, with the effective %-back shown live); tobacco, vape, alcohol, lottery, gift cards, and fuel are excluded from earning and redeeming. Every earn and redemption is a **signed, append-only ledger line** written only by the server — and because DuoCount also holds the countersigned cash-sales totals, the pattern alerts can catch points fraud (points outpacing sales, multi-earn days, redemption bursts) that standalone loyalty tools can't see. Customers check their own balance at **/rewards** with just the store code and their number; the owner sees the outstanding points liability in dollars.

**Scan instead of type.** The phone camera the app already runs on doubles as a barcode scanner: staff can scan a scratch-off pack number or an inventory item's barcode instead of typing it. Scanning is an input accelerator — a form still saves only when the count is signed. (The one exception is the scratch **Scan to log** flow, where each scan *is* a signed reading logged on the spot — that immediacy is its point.)

**One shared, live log.** Every count appears instantly for the whole team, stamped with who logged it and when. Entries can never be edited or deleted — the record is permanent, which is what makes it trustworthy. (The **Log** tab shows the cash and backroom ledger; scratch-off counts read out in the Scratch tab's History and the owner's Scratch report.)
...
**Dashboard.** Totals and trends: net over/short, how many short counts, daily charts, cash sales trend, and breakdowns by employee, drawer, and item. (Scratch-off charts — sales over time, top games, by staff — live in the owner's Scratch report.)
...
**Admin & export.** Managers add staff, set roles, reset PINs, and manage locations, cash drawers, and backroom items. (Scratch packs need no registration at all — a pack exists the first time someone counts it; an optional owner game list just auto-fills names and prices.) Deactivating a staff member cuts their access right away — they're signed out and can no longer log anything, even if their phone still had the app open. (A store always keeps at least one active owner — the app won't let you demote or deactivate the last one.) The owner sets the business name, logo, and sharing mode — and a settings save lands on every signed-in device immediately. Anyone can export the cash/backroom log as a spreadsheet (CSV); scratch detail exports from the owner's Scratch report.
