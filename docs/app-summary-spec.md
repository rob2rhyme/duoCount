...
**Admin & export.** Managers add staff, set roles, reset PINs, and manage locations, drawers, tracked inventory items, and the scratch-off pack registry. Deactivating a staff member cuts their access right away — they're signed out and can no longer log anything, even if their phone still had the app open. The owner sets the business name, logo, and sharing mode. Anyone can export the full log as a spreadsheet (CSV).
