...
**Admin & export.** Managers add staff, set roles, reset PINs, and manage locations, drawers, tracked inventory items, and the scratch-off pack registry. The owner sets the business name, logo, and sharing mode. Anyone can export the full log as a spreadsheet (CSV).
