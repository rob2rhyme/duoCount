...
4. EOD report and CSV show a blind indicator per entry.
...
**Screens.**
- New top-level tab **Notes** (tab bar becomes Cash / Scratch-offs / Log / Notes / Dashboard / Admin; it already h-scrolls on small screens).
...
1. Header: vendor logo + name, store code, location, date, "Generated by {name} at {timestamp}".
...
3. Logo failure falls back to text header (reuse the Logo fallback philosophy).
...
**Digest contents:** per location — entry count, cash sales logged, net over/short (colored), shorts count, scratch dollars; then open variances, open disputes, unverified counts; closing line links to the app. Plain-text part + minimal HTML table; no attachments (the PDF stays on-demand).
