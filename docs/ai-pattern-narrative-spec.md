...
**Status: Phases 1 & 2 built (shipped dark).** Built — the reused digest core
factored into a shared `runNarrative(redacted, ctx)` (behavior-preserving for the
cron), `redactForModel` reused as-is, the pure `buildInsightSummary(patterns,
counts)` + `aiInsightsEnabled` gate, the manager-gated `POST
/api/pattern-narrative` route (server-enforced opt-in, redacts names server-side),
the "Explain these signals" button on the Dashboard's Patterns card (shown only
when `vendor.aiInsights` is on; result cached per pattern-set; React auto-escapes
the model text), the tests (`npm run test:insight`; the digest's 9 tests still
pass), **the `vendor.aiInsights` owner toggle in Admin → Business settings
(grouped with the digest and search toggles) + the one-key `firestore.rules`
allow-list entry, and the `privacy-and-data.md` + `getting-started.md` /
`app-summary-spec.md` disclosures.** Off by default — with the flag off the
Dashboard is unchanged. **Still open:** live model-call verification against a
pilot (needs a real key + deploy; the rules change also wants an emulator/staging
check — `npm run test:rules`). This is the build spec for the third AI feature
named in `ai-features-spec.md` (§Phasing) and `distribution-analysis.md` §1.1.
...
1. **Phase 1 — mechanics + button, shipped dark. ✅ built.** `buildInsightSummary`,
   ...
   in-repo).
2. **Phase 2 — owner UX + disclosure. ✅ built.** The `vendor.aiInsights` toggle in
   Admin → Business settings ("AI insight on the Dashboard — off by default") with
   the data-handling note, grouped with the other AI toggles, the one-key
   `firestore.rules` allow-list entry, and the disclosures in
   `privacy-and-data.md`, the `getting-started` guide (§6 Optional AI helpers), and
   `app-summary-spec.md`.
